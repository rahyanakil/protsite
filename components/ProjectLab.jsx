...
const PROJECTS = [
  {
    ...
    outcome: 'Role-specific server-rendered dashboards with authenticated Stripe checkout and a live AI tutor. Currently in active use.',
  ...
  },
  {
    ...
    solution: 'Gemini AI copilot grounded in live store data via 6 parallel DB queries — it physically cannot hallucinate inventory numbers. One dashboard for sales, inventory, staff, and AI-ranked restock urgency.',
    decisions: [
      'Edge Middleware RBAC (Admin/Owner/Cashier) runs before React hydrates — eliminates flash of protected content',
      'Grounded the AI copilot with a real-time Prisma snapshot injected as system instruction, not user message',
    ...
    stack: ['Next.js 15', 'TypeScript', 'Tailwind CSS', 'ShadCN UI', 'TanStack Query', 'Recharts', 'Gemini AI', 'Zustand', 'Prisma'],
  ...
  },
  {
    ...
    status: 'DEPLOYED',
    statusColor: 'text-[#00d4ff] border-[rgba(0,212,255,0.3)] bg-[rgba(0,212,255,0.08)]',
    problem: "Vercel silently drops session cookies when frontend and backend live on different domains — users log in, get redirected, then immediately hit 401. A login-flicker bug that's hard to reproduce in dev.",
    solution: 'Dual-auth strategy: JWT in localStorage as the reliable path, cookie as fallback. Axios request interceptor attaches Authorization: Bearer on every call — browser cookie policy becomes irrelevant.',
    decisions: [
    ...
    challenges: [
      ...
      'Framer Motion useScroll loses sync when Lenis hijacks native wheel events — synthetic event dispatch fixed this',
      'useSearchParams() caused prerender failures — discovered during build, not dev, requiring Suspense wrappers',
    ],
    outcome: 'Premium UI with mouse-parallax hero, 3D card tilt, Stripe checkout, and role dashboards for Buyer, Agent, Admin. Zero auth flicker.',
    stack: ['Next.js 15', 'TypeScript 5', 'Tailwind CSS', 'Framer Motion', 'TanStack Query v5', 'Stripe', 'React Hook Form', 'Zod', 'Lenis'],
  ...
  },
  {
    ...
    problem: 'Appointment bookings managed by phone and paper — no confirmation, no tracking, high no-show rate, no data for administrators.',
    ...
    decisions: [
      ...
      'State-machine approach for booking workflow (pending → confirmed → completed/cancelled) to prevent invalid transitions',
      'Nodemailer triggered on state changes rather than polling — event-driven, no cron job needed',
    ],
    challenges: [
      'Appointment conflict detection: concurrent requests could double-book the same slot without a proper atomic check',
      'Email trigger reliability: Nodemailer needs SMTP credentials that vary per environment — careful env management',
      'Content categories varied significantly — required a flexible schema, not a rigid normalized table',
    ],
    outcome: 'Full appointment digitization for an active educational institution. Online booking, automated email confirmations, admin analytics.',
                ...
                <div className="space-y-5">
                  ...
                  <div className="os-panel p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {project.stack.map(s => (
                        <span key={s} className="font-mono text-[11px] px-2.5 py-1 rounded border border-os-border text-os-text-muted">
                          {s}
                        </span>
                      ))}
                    </div>
